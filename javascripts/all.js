...
$(document).ready(function(){
	...
	$checkbox = $('#js_checkbox');
	$checkbox.scrollspy({
		min: $checkbox.offset().top - 400,
		max: $checkbox.offset().top,
    onEnter: function(element, position) {
      $checkbox.addClass('end-pos');
    }
	});

	$star = $('#js_star');
	$star.scrollspy({
		min: $star.offset().top - 300,
		max: $star.offset().top,
    onEnter: function(element, position) {
      $star.addClass('end-pos');
    }
	});

	$balloons = $('#js_balloons');
	$balloons.scrollspy({
		min: $balloons.offset().top - 300,
		max: $balloons.offset().top,
    onEnter: function(element, position) {
      $balloons.addClass('end-pos');
    }
	});

	$centered = $('#js_centered');
	$centered.scrollspy({
		min: $centered.offset().top - 300,
		max: $centered.offset().top,
    onEnter: function(element, position) {
      $centered.addClass('end-pos');
    }
	});

	$a_color = $('#js_a_color');
	$a_color.scrollspy({
		min: $a_color.offset().top - 300,
		max: $a_color.offset().top,
    onEnter: function(element, position) {
      $a_color.addClass('end-pos');
    }
	});

	// $toolbar = $('#js_toolbar');
	// $toolbar.scrollspy({
	// 	min: $toolbar.offset().top - 300,
	// 	max: $toolbar.offset().top,
 //    onEnter: function(element, position) {
 //      $toolbar.addClass('end-pos');
 //    }
	// });

	$toggles = $('#js_type-toggles');
	$toggles.scrollspy({
		min: $toggles.offset().top - 300,
		max: $toggles.offset().top,
    onEnter: function(element, position) {
      $('.type-toggle').addClass('wiggle');
    }
	});

	$.each($('.come-in'), function(el) {
		$.proxy($(this).scrollspy({
			min: $(this).offset().top - 300,
			max: $(this).offset().top,
	    onEnter: function(element, position) {
	      $(this).addClass('end-pos');
	    }
		}));
	});
